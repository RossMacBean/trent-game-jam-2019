...
public class FungusTrigger : MonoBehaviour, IInteractive
{
	...
	public string[] tagsThatCanTrigger = { "Player" };
	...
	{
		if (!TriggerOnInteract && IsTaggedCorrectly(other.gameObject)) { Trigger(); }
	}

	void OnCollisionEnter(Collision collision)
	{
		if (!TriggerOnInteract && IsTaggedCorrectly(collision.gameObject)) { Trigger(); }
	}

	void OnControllerColliderHit(ControllerColliderHit hit)
	{
		if (!TriggerOnInteract && IsTaggedCorrectly(hit.collider.gameObject)) { Trigger(); }
	}
